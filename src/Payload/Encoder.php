...
class Encoder extends AbstractPayload
{
    ...
    public function encode()
    {
        ...
        if (0xFFFF < $length) {
            $pack   = pack('NN', ($length & 0xFFFFFFFF00000000) >> 0x20, $length & 0x00000000FFFFFFFF);
			$length = 0x007F;
        ...
        $payload = ($this->fin << 0x1) | $this->rsv[0];
        $payload = ($payload   << 0x1) | $this->rsv[1];
        $payload = ($payload   << 0x1) | $this->rsv[2];
        $payload = ($payload   << 0x4) | $this->opCode;
        $payload = ($payload   << 0x1) | $this->mask;
        $payload = ($payload   << 0x7) | $length;
